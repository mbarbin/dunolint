...
> **Warning:** Beware, although this selector share sub selectors with the *executables* stanzas, its use only selects the *executable* stanza singular.
...
> **Warning:** The `(executables _)` stanzas are currently not handled, and are ignored by dunolint. We have plans to fix this early on.
...
> **Tip:** Use `if_present` when you want a constraint to apply only to existing fields without trying to create them. This is particularly useful for enforcing naming conventions like prefixes or suffixes on optional fields.
...
All fields from the generic *has_field* selector that are marked as applicable to *library* stanzas are available here. See the [has_field](#has-field) section for details on common fields like *instrumentation*, *lint*, *name*, *preprocess*, and *public_name*.
