...
let%expect_test "Predicate.equal" =
  let equal = Dune.Executable.Name.Predicate.equal in
  let equals_a = `equals (Dune.Executable.Name.v "main_a") in
  let equals_b = `equals (Dune.Executable.Name.v "main_b") in
  ...
  require
    (equal
       (`equals (Dune.Executable.Name.v "main_a"))
       (`equals (Dune.Executable.Name.v "main_a")));
  [%expect {||}];
...
let%expect_test "of_string" =
  let test str =
    match Dune.Executable.Name.of_string str with
    | Ok name -> print_s (List [ Atom "Ok"; Dune.Executable.Name.sexp_of_t name ])
    | Error (`Msg msg) -> print_s (List [ Atom "Error"; List [ Atom "Msg"; Atom msg ] ])
  in
  test "";
  [%expect {| (Error (Msg "\"\": invalid Dunolint.Executable.Name")) |}];
  test "main";
  [%expect {| (Ok main) |}];
  test "main-dash";
  [%expect {| (Ok main-dash) |}];
  ...
  [%expect {| (Ok main.dot) |}];
  test "main#sharp";
  [%expect {| (Error (Msg "\"main#sharp\": invalid Dunolint.Executable.Name")) |}];
  test "main@at";
  [%expect {| (Error (Msg "\"main@at\": invalid Dunolint.Executable.Name")) |}];
  ()
;;

let%expect_test "predicate" =
  let test p = Common.test_predicate (module Dune.Executable.Name.Predicate) p in
  test (equals (Dune.Executable.Name.v "main"));
  [%expect {| (equals main) |}];
