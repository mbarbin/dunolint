(*_***************************************************************************)
(*_  dunolint-myers - Myers diff computation and unified-diff printing       *)
...
(*_ Notice: The unified-diff renderer and the [Equal] / [Line] / [compute] API
  ...
  logic — [type hunk], [lines_of_string], [hunks_of_lines], [diff] — derives
  from windtrap.
  ...
  - Added an optional [?color:bool] flag to [diff] for ANSI coloring (cyan
    hunk headers, red deletions, green insertions).
  - [print_diff] is not exposed (this project only uses [diff]). *)
