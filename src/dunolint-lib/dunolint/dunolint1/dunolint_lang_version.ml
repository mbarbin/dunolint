...
(* This module implements the dunolint lang version type and its predicates for
   use in dunolint configuration files (e.g., "(lang dunolint 1.0)").

   The implementation is intentionally similar to [Dune_lang_version] which
   handles the analogous "(lang dune X.Y)" stanza in dune-project files. Both
   modules share the same structure: a version type represented as (major, minor)
   pairs, and predicates using binary comparison operators (=, !=, >, >=, <, <=).
   ...
   - [Dune_lang_version] carries deprecated operator aliases (e.g., "equals",
     "greater_than_or_equal_to") for backward compatibility, which we prefer not
     to introduce here since dunolint_lang_version is new.

   We acknowledge the potential for code sharing in the future, with the exact
   approach and execution left as future work once the designs stabilize. *)
