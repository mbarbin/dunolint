...
let%expect_test "read/write" =
  let test contents =
    Err.For_test.protect (fun () ->
      ...
      let t =
        try Dune_project_linter.Name.read ~sexps_rewriter ~field with
...
let%expect_test "sexp_of" =
  let sexps_rewriter, field = Common.read {| (name my_project) |} in
  let t = Dune_project_linter.Name.read ~sexps_rewriter ~field in
...
let%expect_test "eval" =
  ...
  [%expect {| |}];
  is_true (Dune_project_linter.Name.eval t ~predicate:(`is_prefix "my_"));
  [%expect {| |}];
  is_false (Dune_project_linter.Name.eval t ~predicate:(`is_prefix "other_"));
  [%expect {| |}];
  is_true (Dune_project_linter.Name.eval t ~predicate:(`is_suffix "_project"));
  [%expect {| |}];
  is_false (Dune_project_linter.Name.eval t ~predicate:(`is_suffix "_other"));
  [%expect {| |}];
  ()
;;
