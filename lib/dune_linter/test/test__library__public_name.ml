...
let%expect_test "read/write" =
  let test contents =
    Err.For_test.protect (fun () ->
      ...
      let t = Dune_linter.Library.Public_name.read ~sexps_rewriter ~field in
      print_s (Dune_linter.Library.Public_name.write t))
  in
...
let%expect_test "sexp_of" =
  let sexps_rewriter, field = Common.read {| (public_name pub_name) |} in
  let t = Dune_linter.Library.Public_name.read ~sexps_rewriter ~field in
...
module Predicate = struct
  ...
  type t = Dune.Library.Public_name.Predicate.t as 'a
    constraint
      'a =
      [ `equals of Dune.Library.Public_name.t
      | `is_prefix of string
      | `is_suffix of string
      ]
end
...
let%expect_test "eval" =
  ...
  let t = Dune_linter.Library.Public_name.read ~sexps_rewriter ~field in
  is_true
    (Dune_linter.Library.Public_name.eval
       t
       ~predicate:(`equals (Dune.Library.Public_name.v "pre_hello_suf")));
  [%expect {||}];
  is_false
    (Dune_linter.Library.Public_name.eval
       t
       ~predicate:(`equals (Dune.Library.Public_name.v "hello_suf")));
  [%expect {||}];
  is_true (Dune_linter.Library.Public_name.eval t ~predicate:(`is_prefix "pre_"));
  [%expect {||}];
  is_false (Dune_linter.Library.Public_name.eval t ~predicate:(`is_prefix "hello"));
  [%expect {||}];
  is_true (Dune_linter.Library.Public_name.eval t ~predicate:(`is_suffix "_suf"));
  [%expect {||}];
  is_false (Dune_linter.Library.Public_name.eval t ~predicate:(`is_suffix "hello"));
  [%expect {||}];
  ()
;;
...
let%expect_test "enforce" =
  ...
  let enforce conditions =
    ...
    Dunolinter.Handler.raise ~f:(fun () ->
      List.iter conditions ~f:(fun condition ->
        Dune_linter.Library.Public_name.enforce t ~condition);
      Dune_linter.Library.Public_name.rewrite t ~sexps_rewriter ~field;
  ...
  enforce [ equals (Dune.Library.Public_name.v "pre_hello_suf") ];
  [%expect {| (public_name pre_hello_suf) |}];
  (* Enforcing the equality with a new value changes it. *)
  enforce [ equals (Dune.Library.Public_name.v "new_name") ];
  ...
  enforce
    [ if_
        (is_prefix "false")
        (not_ (is_prefix "pre_"))
        (equals (Dune.Library.Public_name.v "not_equal"))
    ];
